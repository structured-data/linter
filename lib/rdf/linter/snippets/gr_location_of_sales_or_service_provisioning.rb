...
module RDF::Linter
  LINTER_HAML.merge!({
    RDF::URI("http://purl.org/goodrelations/v1#LocationOfSalesOrServiceProvisioning") => {
      :identifier => "gr:LocationOfSalesOrServiceProvisioning",
      ...
      :nested_props => [
        "http://purl.org/goodrelations/v1#legalName", "http://purl.org/goodrelations/v1#name"
      ],
      :property_value => %(
        - if res = yield(object)
          != res
